 ...
 * but immediately the child abandons the program of the parent and starts executing 
 * the function supplied by the single argument. Fork should return pid of child process (SpaceId). Notice this definition is slightly different from the one in the syscall.h file in Nachos. Also, the semantics is not the same as Unix fork(). After forked function func finishes, the control should go back to the instruction after the initial system call Fork.
 * Old definition: void Fork(void (*func)());
*/
